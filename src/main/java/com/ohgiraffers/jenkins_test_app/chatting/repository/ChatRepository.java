...
public interface ChatRepository extends JpaRepository<Messages, Integer>
{
    ...
    @Query(value = "SELECT m.id, m.user_id, m.chatroom_id, m.message_contents, m.send_time, m.is_read FROM messages m JOIN (SELECT chatroom_id, MAX(send_time) AS latest_time FROM messages GROUP BY chatroom_id) lm ON m.chatroom_id = lm.chatroom_id AND m.send_time = lm.latest_time ORDER BY m.send_time DESC", nativeQuery = true)
    List<Messages> selectRecentMessages(@PathVariable("userId") int userId);
